...
class ToolEnvironmentOS:
    ...
    def _record_metric(self, tool_name: str, ok: bool, duration: float, tokens: int = 100) -> None:
        try:
            ...
            if mc is None:
                return
            mc.record_plane(
                query_id=f"tool-{tool_name}",
                plane_name=tool_name,
                tokens_used=tokens,
    ...
    async def execute_tool(
        ...
        try:
            ...
            self._record_metric(
                tool_name, True, duration, getattr(tool, "estimated_cost_tokens", 100)
            )
            try:
            ...
            if tool.has_side_effects and tool_name in (
                "write_file",
                "edit_file",
                "apply_patch",
                "execute_shell",
            ...
            try:
                ...
                pass
            self._record_metric(
                tool_name, False, duration, getattr(tool, "estimated_cost_tokens", 100)
            )
            return {
                "success": False,
                "tool": tool_name,
    ...
    def _edit_file(
        ...
        if target is None or replacement is None:
            raise ValueError(
                "Either (target_content, replacement_content) or (old_str, new_str) must be provided"
            )
        p = Path(self.workspace_root) / path
        if not p.exists():
            raise FileNotFoundError(f"File not found: {path}")
    ...
    def _grep_search(
        self, query: str, path: str = ".", is_regex: bool = False, case_sensitive: bool = True
    ) -> list[dict[str, Any]]:
        target_dir = Path(self.workspace_root) / path
    ...
    def _execute_shell(
        self, command: str, timeout: float = 30.0, cwd: Optional[str] = None
    ) -> dict[str, Any]:
        verdict, reason, risk = self.safety_kernel.evaluate_action(
        ...
        if verdict == SafetyVerdict.BLOCK:
            raise PermissionError(
                f"SafetyKernel blocked command execution: {reason} (risk: {risk})"
            )
    ...
    def _git_status(self) -> str:
        proc = subprocess.run(
            ["git", "status", "--short"], cwd=self.workspace_root, capture_output=True, text=True
        )
        return proc.stdout.strip()

    def _git_diff(self) -> str:
        proc = subprocess.run(
            ["git", "diff"], cwd=self.workspace_root, capture_output=True, text=True
        )
        return proc.stdout.strip()

    def _compact_context(
        self, text: str = "", path: str = "", max_chars: int = 12000
    ) -> dict[str, Any]:
        ...
        rep = ContextCompactor(workspace_root=self.workspace_root, max_chars=max_chars).compact(
            text
        )
    ...
    def connect_mcp_client(
        self, mcp_client: Any, server_name: str, capability_registry: Optional[Any] = None
    ) -> list[str]:
        """
