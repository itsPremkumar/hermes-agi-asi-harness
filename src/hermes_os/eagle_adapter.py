...
class EagleClaim:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {"title": self.title, "url": self.url, "snippet": self.snippet[:500],
                "backend": self.backend, "confidence": self.confidence,
    ...
    for it in items[:limit]:
        try:
            if isinstance(it, dict):
                ...
                claims.append(EagleClaim(
                    title=str(it.get("title") or it.get("name") or url)[:160],
                    url=url, snippet=str(it.get("snippet") or it.get("body") or it.get("summary") or "")[:800],
                    backend=backend,
                    confidence=float(it.get("confidence", 0.6))))
            else:
                claims.append(EagleClaim(title=str(it)[:80], url="", snippet=str(it), backend=backend))
        except Exception:
            continue
    return claims
...
def _backend_table() -> Dict[str, Callable[..., Any]]:
    ...
    try:
        ...
        for attr, name in (("_wikipedia_search_wrapper", "wikipedia"),
...
class EagleAdapter:
    ...
    def _fanout(self, query: str, limit: int, backends: Optional[List[str]] = None,
                strict: bool = False) -> List[EagleClaim]:
    ...
    def fetch_extract(self, url: str, strict: bool = False) -> Optional[EagleClaim]:
        ...
        try:
            ...
            self.queries += 1
            return EagleClaim(title=url[:120], url=url, snippet=str(text)[:1500],
                              backend="fetch", confidence=0.55)
        except Exception as e:
    ...
    def register_capabilities(self, registry: Any) -> List[str]:
        ...
        try:
            ...
            for cid, name, desc, best in (
                ("eagle.web_search", "Eagle Web Search",
                 "Parallel multi-backend web search with provenance", ["market_analysis", "domain_recon"]),
                ("eagle.academic_search", "Eagle Academic Search",
                 "arXiv/Wikipedia evidence with citations", ["literature_review", "fact_check"]),
                ("eagle.fetch_extract", "Eagle Fetch+Extract",
                 "Bounded URL fetch with readability extraction", ["source_verification", "evidence_packet"]),
            ):
                registry.register(CapabilityManifest(
                    id=cid, kind=CapabilityKind.SKILL, name=name, description=desc,
                    inputs=["query_or_url"], outputs=["evidence_packet", "verified_claims"],
                    risk="low", best_for=best))
                ids.append(cid)
            return ids
        except Exception as e:
    ...
    def as_tools(self, tool_env: Any) -> List[str]:
        ...
        try:
            ...
            for tname, desc, handler, tokens in defs:
                tool_env.register(ToolDescriptor(
                    name=tname, description=desc, handler=handler,
                    required_permission="read", risk_level="low",
                    estimated_cost_tokens=tokens, sandbox_required=False))
                names.append(tname)
            return names
        except Exception as e:
    ...
    def stats(self) -> Dict[str, Any]:
        return {"enabled": self.enabled, "queries": self.queries,
                "elapsed_total": round(self.elapsed_total, 2),
                "hits": dict(self.backend_hits), "fails": dict(self.backend_fails)}
    ...
    def health(self) -> Dict[str, Any]:
        ...
        for name in sorted(set(list(table) + list(self.backend_hits) + list(self.backend_fails))):
            ...
            if name not in table:
                ...
                status = "healthy"
            rows[name] = {"status": status, "hits": hits, "fails": fails,
    ...
    def mcp_specs(self) -> List[Dict[str, Any]]:
        ...
        return [
            {"name": "eagle_web_search", "description": "Parallel governed web search with provenance",
             "input_schema": {"type": "object", "properties": {
                 "query": {"type": "string"}, "limit": {"type": "integer", "default": 8}}},
             "risk": "low", "side_effects": False},
            {"name": "eagle_academic_search", "description": "Academic evidence search with citations",
             "input_schema": {"type": "object", "properties": {
                 "query": {"type": "string"}, "limit": {"type": "integer", "default": 6}}},
             "risk": "low", "side_effects": False},
            {"name": "eagle_fetch", "description": "Bounded fetch+extract of one URL (tainted)",
             "input_schema": {"type": "object", "properties": {"url": {"type": "string"}}},
             "risk": "medium", "side_effects": False},
        ]
