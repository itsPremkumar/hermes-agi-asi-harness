...
_KEEP_PATTERNS = (
    "invariant", "decision", "error", "fail", "verdict", "proof",
    "warning", "critical", "rollback", "checkpoint", "approved",
)
...
def _is_keep(line: str) -> bool:
    low = line.lower()
    return any(k in low for k in _KEEP_PATTERNS) or line.strip().startswith(("#", "##", "-", "*", "1."))
...
class ContextCompactor:
    ...
    def compact(self, text: str, label: str = "context") -> Dict[str, Any]:
        ...
        if not self.needs_compaction(original):
            return {"compacted": original, "original_chars": len(original),
                    "compacted_chars": len(original), "digest_lines": 0, "archive": None,
        ...
        archive_name = f"{label}-{int(time.time())}-{hashlib.sha256(original.encode()).hexdigest()[:8]}.txt"
        ...
        compacted = "\n".join(
            ["# COMPACTED CONTEXT (full text archived)",
             ...
             "", "## SIGNAL"] + kept + ["", "## DIGEST"] + digest + ["", "## TAIL"] + tail)
        return {"compacted": compacted, "original_chars": len(original),
                "compacted_chars": len(compacted), "digest_lines": len(digest),
                "archive": str(archive) if archive else None, "compacted_flag": True}
