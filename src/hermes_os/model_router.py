...
class ModelPortfolio:
    ...
    def _register_defaults(self) -> None:
        for m in (
            ModelEntry(
                "hermes_managed", "reasoning", quality=0.90, cost_per_1k=0.0, avg_latency_s=15.0
            ),
            ModelEntry("hermes_local", "coding", quality=0.82, cost_per_1k=0.0, avg_latency_s=8.0),
            ModelEntry(
                "frontier_reasoner", "reasoning", quality=0.95, cost_per_1k=0.01, avg_latency_s=20.0
            ),
            ModelEntry(
                "coding_model", "coding", quality=0.88, cost_per_1k=0.006, avg_latency_s=12.0
            ),
    ...
    def _save(self) -> None:
        try:
            ...
            self._file.write_text(
                json.dumps({m: e.to_dict() for m, e in self._models.items()}, indent=2),
                encoding="utf-8",
            )
        except Exception:
            pass
    ...
    def route(self, task_kind: str = "general", budget: str = "balanced") -> ModelEntry:
        ...
        if hermes_up:
            prefer = {
                "reasoning": "hermes_managed",
                "coding": "hermes_local",
                "general": "hermes_managed",
            }.get(task_kind)
        ...
        pool = {
            mid: m for mid, m in self._models.items() if hermes_up or not mid.startswith("hermes_")
        }
        if not pool:
        ...
        scored = sorted(
            cands,
            key=lambda m: (
                m.quality * max(0.2, m.success_rate) - m.cost_per_1k * 10 - m.avg_latency_s / 100
            ),
            reverse=True,
        )
