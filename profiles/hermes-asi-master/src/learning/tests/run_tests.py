"""Run all tests and write results to test_results.txt."""
...
def _():
    ev = SelfEvaluator()
    ev.review(goal="g", prediction="p", outcome="o", success=True, capability="x")
    ev.reviews[0] = Review(goal="g", prediction="p", outcome="o", success=True, confidence=0.5, timestamp=1000.0)
...
def _():
    s = Skill(name="write", description="write file", template="Write {content} to {path}",
...
def _():
    s = Skill(name="write", description="d", template="Write {content} to {path}",
...
def _():
    s = Skill(name="greet", description="d", template="Hello {name}, welcome to {place}",
              parameters=[SkillParameter(name="name", required=True), SkillParameter(name="place", default="Earth")])
...
def _():
    s = Skill(name="test", description="desc", template="Do {x}",
              parameters=[SkillParameter(name="x")], version=3, test_pass_rate=0.9)
    d = s.to_dict()
...
def _():
    f = SkillForge()
    s = Skill(name="echo", description="d", template="echo {msg}", parameters=[SkillParameter(name="msg")])
...
def _():
    f = SkillForge()
    s = Skill(name="echo", description="d", template="echo {msg}", parameters=[SkillParameter(name="msg")])
    cases = [{"msg": "hi", "expected": "echo hi"}, {"msg": "bye", "expected": "WRONG"}]
    logs = f.test(s, cases)
...
def _():
    f = SkillForge()
    skill, verdict = f.forge_pipeline("full_test", description="d", template="Run {task}",
        parameters=[SkillParameter(name="task")],
        test_cases=[{"task": "a", "expected": "Run a"}], min_pass_rate=0.8)
...
def _():
    ...
    results = [PracticeResult(task_id="t1", success=True, score=0.9), PracticeResult(task_id="t2", success=False, score=0.3)]
...
def _():
    ...
    buf.store(m)
    r = Replayer(buf)
    result = r.compare_runners("m1", {"perfect": lambda o, a: a, "wrong": lambda o, a: "different"})
