...
CENTERPIECE: run_all_benchmarks, get_overall_score, get_category_scores,
get_benchmark_scores, get_improvements, generate_report.
"""
...
class FullEvaluationSuite:
    ...
    def get_overall_score(self) -> float:
        """Get overall weighted score (0-100 scale)."""
        with self._lock:
            ...
            return (total_passed / total_problems) * 100

    def get_category_scores(self) -> dict[str, float]:
        """Get scores grouped by category (reasoning/coding/math/language/bias/safety)."""
        with self._lock:
            ...
            return {
                cat: (sum(scores) / len(scores)) * 100 if scores else 0.0
    ...
    def get_benchmark_scores(self) -> dict[str, float]:
        ...
        with self._lock:
            return {r.benchmark: r.score * 100 for r in self._results}
            ...
            for result in self._results:
                if result.score < 0.5:
                    improvements.append({
                        ...
                        "priority": "high",
                        "recommendation": f"Focus on improving {result.benchmark} performance",
                    })
                    ...
                    improvements.append({
                        ...
                        "current_score": result.score * 100,
                        "priority": "medium",
                        "recommendation": f"Refine {result.benchmark} strategies",
                    })
            return improvements
    ...
    def generate_report(self) -> EvalReport:
        ...
        with self._lock:
            ...
            overall_score = (total_passed / total_problems) * 100 if total_problems else 0.0
            ...
            report = EvalReport(
                ...
                benchmark_scores=self.get_benchmark_scores(),
...
def build_default_full_evaluation_suite() -> FullEvaluationSuite:
    """Build evaluation suite with all 14 benchmarks."""
    ...
    try:
        ...
        suite.register_benchmark("SWE-bench Pro", SWEBenchPro(), "coding")
    except ImportError:
        pass
    ...
    try:
        ...
        suite.register_benchmark("PIQA", PIQABenchmark(), "reasoning")
    except ImportError:
        pass

    try:
        from benchmark.openbookqa_benchmark import OpenBookQABenchmark
        suite.register_benchmark("OpenBookQA", OpenBookQABenchmark(), "reasoning")
    except ImportError:
        pass

    try:
        from benchmark.siqa_benchmark import SIQABenchmark
        suite.register_benchmark("SIQA", SIQABenchmark(), "reasoning")
    except ImportError:
        pass

    try:
        from benchmark.wino_grande_benchmark import WinoGrandeBenchmark
        suite.register_benchmark("WinoGrande", WinoGrandeBenchmark(), "reasoning")
    except ImportError:
        pass

    # Math benchmarks
    try:
        from benchmark.gsm8k_benchmark import GSM8KBenchmark
        suite.register_benchmark("GSM8K", GSM8KBenchmark(data_dir="./data"), "math")
    except (ImportError, TypeError, OSError):
        pass

    # Language benchmarks
    try:
        from benchmark.mmlu_benchmark import MMLUBenchmark
        suite.register_benchmark("MMLU", MMLUBenchmark(data_dir="./data"), "language")
    except (ImportError, TypeError, OSError):
        pass

    # ARC-AGI-3
    try:
        from benchmark.arc_agi_3_full_eval import ARCAGI3FullEval
        suite.register_benchmark("ARC-AGI-3", ARCAGI3FullEval(), "reasoning")
    except (ImportError, TypeError, OSError):
        pass

    # Bias benchmarks
    try:
        from benchmark.winogender_benchmark import WinogenderBenchmark
        suite.register_benchmark("Winogender", WinogenderBenchmark(), "bias")
    except (ImportError, TypeError, OSError):
        pass

    # Safety benchmarks
    try:
        from benchmark.real_toxicity_prompts_benchmark import RealToxicityPromptsBenchmark
        suite.register_benchmark("RealToxicityPrompts", RealToxicityPromptsBenchmark(), "safety")
    except (ImportError, TypeError, OSError):
        pass
